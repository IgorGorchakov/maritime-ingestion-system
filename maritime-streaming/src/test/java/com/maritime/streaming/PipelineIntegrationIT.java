package com.maritime.streaming;
...
class PipelineIntegrationIT {
    ...
    void setUpClients() {
        ...
        enrichedConsumer.subscribe(List.of("maritime.enriched"));
        ...
        quarantineConsumer.subscribe(List.of("maritime.ais.quarantine"));
    }
    ...
    void validEvent_appearsOnEnrichedTopic() {
        ...
        producer.send(new ProducerRecord<>("maritime.ais.raw", event.getMmsi(), event));
        producer.flush();
    ...
    void invalidMmsi_routedToQuarantineTopic() {
        VesselEvent bad = VesselEvent.newBuilder()
                ...
                .build();
        producer.send(new ProducerRecord<>("maritime.ais.raw", "INVALID", bad));
        producer.flush();
    ...
    void nullIsland_routedToQuarantine() {
        VesselEvent nullIsland = VesselEvent.newBuilder()
                ...
                .setTimestamp(Instant.now()).setEventType("AIS")
                .build();
        producer.send(new ProducerRecord<>("maritime.ais.raw", nullIsland.getMmsi(), nullIsland));
        producer.flush();
    ...
    void duplicateEvent_enrichedOnlyOnce() {
        ...
        ProducerRecord<String, VesselEvent> record =
                new ProducerRecord<>("maritime.ais.raw", event.getMmsi(), event);
