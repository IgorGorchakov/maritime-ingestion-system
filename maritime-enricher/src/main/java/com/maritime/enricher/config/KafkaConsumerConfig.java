 ...
 * </ul>
 *
 * <p>All tunable values ({@code bootstrapServers}, {@code schemaRegistryUrl},
 * {@code groupId}) are injected via {@link EnricherKafkaProperties} — a typed
 ...
 * defaults are removed, including the contradicted {@code "enricher-service"}
 * group-id default that disagreed with the {@code "streaming-service"} value in
 * {@code application.properties}.
 */
@Configuration
public class KafkaConsumerConfig {

    private final EnricherKafkaProperties kafka;

    public KafkaConsumerConfig(EnricherKafkaProperties kafka) {
        this.kafka = kafka;
    }
    ...
    public KafkaAdmin kafkaAdmin() {
        return new KafkaAdmin(Map.of("bootstrap.servers", kafka.bootstrapServers()));
    }
}
